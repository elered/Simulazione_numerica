...
void System :: step(){ //Fa uno step della simulazione
  if(_sim_type == 0) this->Verlet();  //Fa uno step di dinamica molecolare usando l'algoritmo di Verlet
  else for(int i=0; i<_npart; i++) this->move(int(_rnd.Rannyu()*_npart)); //Viene selezionata casualmente una particella, e per questa particella viene eseguito un passo di MC chiamando la funzione move()
  _nattempts += _npart; //Aggiorna il numero totale di tentativi effettuati sul sistema
  return;
}

void System :: Verlet(){
  double xnew, ynew, znew;
  // ciclo che scorre tutte le particelle _npart nel sistema e calcola la forza che agisce su ciascuna particella lungo le tre dimensioni (x, y, z). 
  //Queste forze vengono memorizzate negli array _fx, _fy, _fz, dove _fx(i), _fy(i), _fz(i) rappresentano le componenti delle forze sulla particella i
  for(int i=0; i<_npart; i++){
    _fx(i) = this->Force(i,0);
    _fy(i) = this->Force(i,1);
    _fz(i) = this->Force(i,2);
  }
  // ciclo che scorre tutte le particelle nel sistema e applica lo schema di integrazione di Verlet per aggiornare le posizioni delle particelle
  for(int i=0; i<_npart; i++){
  ...
  _naccepted += _npart; //Tengo traccia del numero di accettazioni
  return;
}

double System :: Force(int i, int dim){
  double f=0.0, dr; //f forza totale e dr distanza tra le particelle
  vec distance;
  distance.resize(_ndim);
  //itero su tutte le coppie di particelle del sistema, calcolo la distanza su tutte le direzioni tra le due particelle usando la pbc
  for (int j=0; j<_npart; j++){
    if(i != j){
      ...
      //se la distanza tra le particelle è minore del raggio di cutoff  viene calcolata la forza tra le particelle utilizzando il potenziale di Lennard-Jones
      if(dr < _r_cut){
...
void System :: move(int i){ //Proporre un movimento di campionamento di Monte Carlo (MC) per una particella specifica i nel sistema
  if(_sim_type == 3){ //Gibbs sampler for Ising
    // TO BE FIXED IN EXERCISE 6
     double tmp = 2.0*_beta*_J*(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin());
     double expon = exp(-tmp)*exp(-2.0*_beta*_H);  
     double prob = 1.0/(1.0+expon);
...
double System :: Boltzmann(int i, bool xnew){ //Calcola l'energia di Boltzmann di una particella specifica i nel sistema
  double energy_i=0.0; //Utilizzata per accumulare l'energia di interazione tra la particella i e le altre particelle nel sistema
  double dx, dy, dz, dr; //Differenze di posizione lungo le tre dimensioni tra la particella i e le altre particelle
  for (int j=0; j<_npart; j++){
    if(j != i){
...
double System :: pbc(double position, int i){ //Applico le condizioni al contorno periodiche alle posizioni delle particelle nel sistema
  return position - _side(i) * rint(position / _side(i));
}

int System :: pbc(int i){ //Utilizzata per applicare le condizioni al contorno periodiche agli indici degli spin in un modello di Ising
...
void System :: initialize_velocities(){
  //Se il flag _restart è attivo e il tipo di simulazione _sim_type è 0 (dinamica molecolare), il programma cerca di aprire il file 
  //"../INPUT/CONFIG/velocities.in" per leggere le velocità iniziali delle particelle. Se il file viene aperto correttamente, 
  //legge le velocità e le imposta per ciascuna particella. Altrimenti, stampa un messaggio di errore
  if(_restart and _sim_type==0){
    ...
    cinf.close();
    //Se non si tratta di un restart o se il tipo di simulazione non è una dinamica molecolare, vengono generate nuove velocità casuali per le particelle
  } else {
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros(); //imposto tutti gli elementi del vettore a zero (è una cosa di Armadillo)
    for (int i=0; i<_npart; i++){
    ...
    //Le velocità vengono corrette sottraendo la somma delle velocità per ogni dimensione e normalizzando la distribuzione delle velocità rispetto alla temperatura desiderata
    ...
    //Le velocità corrette vengono quindi impostate per ciascuna particella nel sistema
    for (int i=0; i<_npart; i++){
  ...
  if(_sim_type == 0){ //Simulazione di dinamica molecolare
  double xold, yold, zold;
    for (int i=0; i<_npart; i++){ 
      //Per ciascuna particella, calcola le posizioni precedenti lungo ciascuna dimensione (xold, yold, zold) 
      //utilizzando le posizioni attuali e le velocità delle particelle
...
void System :: finalize(){
  this->write_configuration(); //scrive la configurazione finale del sistema
  _rnd.SaveSeed(); //salva il seme (mi serve per la riproducibilità)
  ofstream coutf;
  coutf.open("../OUTPUT/output.dat",ios::app);
...
void System :: write_configuration(){
  ofstream coutf;
  //Controlla il tipo di simulazione (_sim_type). Se è inferiore a 2, il sistema è considerato una simulazione di dinamica molecolare
  // o di Monte Carlo per particelle, altrimenti è una simulazione per l'Ising
  if(_sim_type < 2){ 
    ...
    if(coutf.is_open()){
      ...
      //setw(16) è una funzione della libreria <iomanip> in C++ che imposta la larghezza del campo di output. In questo contesto, 
      //imposta la larghezza del campo di output per ciascuna coordinata (x, y, z) a 16 caratteri
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open("../OUTPUT/CONFIG/config_" + to_string(nconf) + ".xyz"); //to_string(nconf) converte il numero di configurazione in una stringa per includerlo nel nome del file
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
      coutf << "LJ" << "  " 
            << setw(16) << _particle(i).getposition(0,true)          // x (qui a differenza di prima sto scrivendo le posizioni senza normalizzazione)
...
void System :: read_configuration(){
  //Legge il numero di coordinate (ncoord) dal file e verifica se corrisponde al numero di particelle nel sistema (_npart).
  //Se i valori non corrispondono, viene stampato un messaggio di errore su cerr e il programma viene terminato
  ifstream cinf;
...
void System :: block_reset(int blk){ // Reset block accumulators to zero
  ofstream coutf;
  if(blk>0){
    coutf.open("../OUTPUT/output.dat",ios::app); //sto aprendo il file in modalità aggiunta, cioè posso aggiungere testo al file senza cancellare i contenuti esistenti
...
void System :: measure(){ // Measure properties
  _measurement.zeros(); //Inizializza un vettore _measurement a zero per memorizzare le misure delle proprietà
  ...
  double virial=0.0;
  if (_measure_penergy or _measure_pressure or _measure_gofr) { //Se _measure_penergy, _measure_pressure o _measure_gofr è vero, calcola l'energia potenziale, il viriale e la funzione di distribuzione radiale (g(r))
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if (_measure_pressure)  virial += (1.0/pow(dr,12) - (0.5)*(1.0/pow(dr,6)));
        }
      }
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){ //faccio la media dell'energia potenziale e la salvo in _measurement(_index_penergy)
    penergy_temp = _vtail + 4.0 * penergy_temp / double(_npart);
    _measurement(_index_penergy) = penergy_temp;
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    for (int i=0; i<_npart; i++) kenergy_temp += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() ); //la funzione dot() calcola il prodotto scalare tra due vettori
  ...
  if (_measure_tenergy){
    ...
    else { //vuol dire che sto facendo il modello di Ising
      double s_i, s_j;
      for (int i=0; i<_npart; i++){
...
// TO BE FIXED IN EXERCISE 6
  ...
  if (_measure_cv )  {
    ...
    double tmp = _measurement(_index_tenergy);
    tmp *= double(_npart);
    double tenergy_temp2 = tmp*tmp;
...
// TO BE FIXED IN EXERCISE 6
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
    ...
    if (_measure_chi) {
      ...
      _measurement(_index_chi) = _beta*magnetization/double(_npart);

    }

  _block_av += _measurement; //Aggiorno gli accumulatori di blocco con le varie misure

  return;
}

void System :: averages(int blk){ //calcola le medie dei valori misurati durante ciascun blocco di simulazione e le registra su file per l'analisi successiva

  ofstream coutf;
  double average, sum_average, sum_ave2;
  double cv_tmp, cv_tmp2;
  if(_measure_cv) {
    cv_tmp = _global_av(_index_cv);
    cv_tmp2 = _global_av2(_index_cv);
  }

  _average     = _block_av / double(_nsteps);
  _global_av  += _average;
  _global_av2 += _average % _average; //per ciascun elemento dei due vettori, viene eseguita una moltiplicazione e il risultato viene inserito in un nuovo vettore

  if(_measure_cv) {
    _global_av(_index_cv) = cv_tmp;
    _global_av2(_index_cv) = cv_tmp2;
  }
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
    ...
    average  = (_beta*_beta*(_average(_index_cv) - double(_npart)*double(_npart)*(_average(_index_tenergy)*_average(_index_tenergy))));
    //DEVI RIUSCIRE AD AGGIORNARE LE MEDIE DI BLOCCO
  ...
  // TO BE FIXED IN EXERCISE 6
  // ACCEPTANCE (calcola la frazione di accettazione delle mosse durante la simulazione e la scrive nel file di output "acceptance.dat")
  double fraction;
...
double System :: error(double acc, double acc2, int blk){ //Calcola l'errore
  if(blk <= 1) return 0.0;
  else return sqrt( fabs(acc2/double(blk) - pow( acc/double(blk) ,2) )/double(blk) );
}

int System :: get_nbl(){ //Restituisce il numero di blocchi usati nella simulazione
  return _nblocks;
}

int System :: get_nsteps(){ //Restituisce il numero di step usati nella simulazione
  return _nsteps;
}
