...
vector<vector<double>> random_walk_average(int num_steps, int num_trials, int num_blocks, double a, Random &rnd) {
  ...
  vector<vector<double>> risultato(3, vector<double>(num_steps));
  ...
  for (int step = 1; step <= num_steps; step++) {
    ...
    for (int block = 0; block < num_blocks; block++) {
      ...
      risultato[0][step - 1] += block_sum;
      risultato[1][step - 1] += block_sum * block_sum;
    }

    // Calcola la media tra i blocchi
    risultato[0][step - 1] /= num_blocks;
    risultato[1][step - 1] /= num_blocks;
    risultato[2][step - 1] = sqrt((risultato[1][step - 1] - risultato[0][step - 1] * risultato[0][step - 1]) / num_blocks);
  }
